...
func TestGenerateCLI(t *testing.T) {
	...
	out, err := RunGenerate(st, "http://10.0.0.1:8443", []string{"--name=DB_Sunucusu"})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(out, "curl -sSL http://10.0.0.1:8443/install/tk_") {
		t.Fatalf("generate output: %q", out)
	}
	// idempotent: same name returns the existing server's command
	out2, err := RunGenerate(st, "http://10.0.0.1:8443", []string{"--name=DB_Sunucusu"})
	if err != nil || out2 != out {
