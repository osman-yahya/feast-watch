...
// server and return the one-liner install command with the mother's public URL
// embedded, so the CLI and the panel print the same command.
func RunGenerate(st *store.Store, publicURL string, args []string) (string, error) {
	...
	return api.InstallCommand(publicURL, srv.Token), nil
}
